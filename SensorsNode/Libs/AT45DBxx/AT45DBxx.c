...
uint8_t AT45dbxx_Spi(uint8_t Data) {
	uint8_t ret = 0;
	HAL_SPI_TransmitReceive(&_AT45DBXX_SPI, &Data, &ret, 1, 100);
	return ret;
}
//################################################################################################################
uint8_t AT45dbxx_ReadStatus(void) {
	...
	HAL_GPIO_WritePin(_AT45DBXX_CS_GPIO, _AT45DBXX_CS_PIN, GPIO_PIN_SET);
	return status;
}
//################################################################################################################
...
void AT45dbxx_WritePage(uint8_t *Data, uint16_t len, uint16_t page) {
	...
	AT45dbxx_Spi((page >> 8) & 0xff);
	AT45dbxx_Spi(page & 0xff);
	HAL_SPI_Transmit(&_AT45DBXX_SPI, Data, len, 100);
	HAL_GPIO_WritePin(_AT45DBXX_CS_GPIO, _AT45DBXX_CS_PIN, GPIO_PIN_SET);
	AT45dbxx_WaitBusy();
}
//################################################################################################################
void AT45dbxx_ReadPage(uint8_t *Data, uint16_t len, uint16_t page) {
	...
	AT45dbxx_Spi(page & 0xff);
	AT45dbxx_Spi(0);
	HAL_SPI_Receive(&_AT45DBXX_SPI, Data, len, 100);
	HAL_GPIO_WritePin(_AT45DBXX_CS_GPIO, _AT45DBXX_CS_PIN, GPIO_PIN_SET);
}
//################################################################################################################
